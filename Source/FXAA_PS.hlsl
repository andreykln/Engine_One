...
static float inverse_screen_size_y = 1.0f / 720.0f;
static float inverse_screen_size_x = 1.0f / 1280.0f;
...
float4 main(VertexOut pin) : SV_TARGET
{
    ...
    {
        color = float4(colorCenter, drawingPass.Sample(smpAnisotropicWrap, pin.texC).w);
        return color;
    }
    ...
    float stepLength = isHorizontal ? inverse_screen_size_y : inverse_screen_size_x;
    ...
    float2 offset = isHorizontal ? float2(inverse_screen_size_x, 0.0) : float2(0.0, inverse_screen_size_y);
	// Compute UVs to explore on each side of the edge, orthogonally. The QUALITY allows us to step faster.
    float2 uv1 = currentUv - offset * QUALITY(0);
    float2 uv2 = currentUv + offset * QUALITY(0);
	
	// Read the lumas at both current extremities of the exploration segment, and compute the delta wrt to the local average luma.
    float lumaEnd1 = rgb2luma(drawingPass.Sample(smpLinearClamp, uv1).rgb);
    float lumaEnd2 = rgb2luma(drawingPass.Sample(smpLinearClamp, uv2).rgb);
    ...
    if (!reachedBoth)
    {
        ...
        for (int i = 2; i < ITERATIONS; i++)
        {
            ...
            if (!reached1)
            {
                lumaEnd1 = rgb2luma(drawingPass.Sample(smpLinearClamp, uv1).rgb);
                lumaEnd1 = lumaEnd1 - lumaLocalAverage;
            }
			// If needed, read luma in opposite direction, compute delta.
            if (!reached2)
            {
                lumaEnd1 = rgb2luma(drawingPass.Sample(smpLinearClamp, uv2).rgb);
