...
VertexOut main(VertexIn vin)
{
    ...
    vout.waveDisplacementTex0 = mul(float4(vin.texC, 0.0f, 1.0f), waveDisplacementTexTransform0).xy;
    vout.waveDisplacementTex1 = mul(float4(vin.texC, 0.0f, 1.0f), waveDisplacementTexTransform1).xy;

    vout.waveNormalTex0 = mul(float4(vin.texC, 0.0f, 1.0f), waveNormalTexTransf0).xy;
    vout.waveNormalTex1 = mul(float4(vin.texC, 0.0f, 1.0f), waveNormalTexTransf1).xy;

    
    // Transform to homogeneous clip space.
    vout.texCoord = mul(float4(vin.texC, 0.0f, 1.0f), texTransform).xy;
