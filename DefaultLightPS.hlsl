...
float4 main(VertexOut pin) : SV_TARGET
{
    ...
    if (disableTexSampling)
   {
	...
	// Interpolating normal can unnormalize it, so renormalize it.
    pin.NormalW = normalize(pin.NormalW);
    float4 normalMapSample = SRVNormalMap.Sample(smpNormalMap, pin.Tex);
	...
	// Dynamically look up the texture in the array.
    diffuseAlbedo *= SRVTexture.Sample(smpLinearWrap, pin.Tex);
    float shininess = (mat.shininess) * normalMapSample.a;
    ...
    float3 r = reflect(-toEyeW, bumpedNormalW);
    float4 reflectionColor = cubeMap.Sample(smpLinearWrap, r);
    float3 fresnelFactor = SchlickFresnel(fresnelR0, bumpedNormalW, r);
    litColor.rgb += shininess * fresnelFactor * reflectionColor.rgb;
    // Common convention to take alpha from diffuse albedo.
    litColor.a = diffuseAlbedo.a;
